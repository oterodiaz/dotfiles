...
function editPlugins()
  path = fn.stdpath('config') .. '/lua/plugins.lua'
  cmd = 'e ' .. path
  vim.cmd(cmd)
end
...
-- api.nvim_create_autocmd({ 'BufEnter', 'BufWinEnter', 'WinEnter', 'CmdwinEnter' }, {
--   desc = 'Hide the statusline while in an NvimTree buffer',
--   group = 'custom',
--   pattern = '*',
--   command = 'if bufname(\'%\') =~# "NvimTree.*" | set laststatus=0 | else | set laststatus=2 | endif'
-- })
...
api.nvim_set_keymap('n', '<Leader>fP',  '',                        { noremap = true, silent = true, callback = editPlugins })
